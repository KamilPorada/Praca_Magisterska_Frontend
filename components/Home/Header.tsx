...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 bg-backgroundColor`}
    >
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="fixed top-0 right-0 w-64 h-full bg-backgroundColor shadow-lg p-6 flex flex-col z-50"
            >
              ...
              {/* Linki w menu */}
              <nav className="flex flex-col items-start space-y-4 mt-8 font-medium">
                {[
                  { label: "O nas", href: "#about" },
                  { label: "Aktualna prognoza", href: "#forecast" },
                  { label: "Historia pogody", href: "#history" },
              ...
              {/* Przycisk logowania w menu */}
              <div className="mt-auto">
                <Button className="w-full mt-6">Zaloguj się</Button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
