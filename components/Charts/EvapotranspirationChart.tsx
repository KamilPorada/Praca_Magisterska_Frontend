...
const EvapotranspirationChart: React.FC<ChartProps> = ({ data }) => {
	...
	const processedData = data.map(item => {
		if (isDaily) {
			return {
				...
				evapotranspiration: (item as DailyWeatherData).evapotranspiration,
			}
		} else if (isMonthly) {
			return {
				name: `${(item as MonthlyWeatherData).year}-${(item as MonthlyWeatherData).month.toString().padStart(2, '0')}`,
				evapotranspiration: (item as MonthlyWeatherData).evapotranspiration,
			}
		}
		return {}
	})
	...
	return (
		<div className='w-1/3 h-96 bg-secondaryColor p-4 rounded-lg shadow-lg'>
			<h2 className='text-sm sm:text-base font-thin text-center text-gray-200 mb-4'>
				Wykres słupkowy ilustrujący ewapotranspiracje
			</h2>
			<ResponsiveContainer width='100%' height='90%'>
				<BarChart data={processedData}>
					...
					/>
					<Bar dataKey='evapotranspiration' fill='#66bb6a' name='Ewapotranspiracja' />
				</BarChart>
			</ResponsiveContainer>
		</div>
	)
